...
namespace CdpSampleWebApi
{
    ...
    public sealed class ArcGisTableProvider : ITableProvider
    {
        ...
        public async Task<TableValue> GetTableValueAsync(string dataset, string tableName, CancellationToken cancellationToken = default)
        {
            ValidateDataset(dataset);
            var layer = await GetLayerAsync(tableName, cancellationToken).ConfigureAwait(false);
        ...
        private async Task<ArcLayer> GetLayerAsync(string tableName, CancellationToken cancellationToken)
        {
            ...
            if (!json.RootElement.TryGetProperty("fields", out var fieldArray))
            {
                throw new InvalidOperationException($"ArcGIS layer has no fields: {url}");
            }
            foreach (var field in fieldArray.EnumerateArray())
            {
                fields.Add(new ArcField(
                    field.GetProperty("name").GetString() ?? throw new InvalidOperationException("Field name missing."),
                    field.GetProperty("type").GetString() ?? "esriFieldTypeString"));
            }
            return new ArcLayer(fields);
        }
        ...
        private sealed record ArcField(string Name, string Type);
        private sealed record ArcLayer(IReadOnlyList<ArcField> Fields);
    }
}
