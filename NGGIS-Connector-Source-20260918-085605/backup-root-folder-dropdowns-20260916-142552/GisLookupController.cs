...
namespace CdpSampleWebApi.Controllers
{
    ...
    public sealed class GisLookupController : ControllerBase
    {
        ...
        /// <summary>
        /// Lists the ArcGIS Web Adaptors used as root selections.
        /// </summary>
        /// <returns>The available root selections.</returns>
        [HttpGet("roots")]
        public LookupResponse GetRoots()
        {
            return new LookupResponse(Adaptors
                .OrderBy(value => value, StringComparer.OrdinalIgnoreCase)
                .Select(value => new LookupItem(value, value))
                .ToArray());
        }

        /// <summary>
        /// Lists immediate folders from only the selected ArcGIS root.
        /// </summary>
        /// <param name="root">The selected ArcGIS Web Adaptor.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>The available folder selections.</returns>
        [HttpGet("folders")]
        public async Task<LookupResponse> GetFolders([FromQuery] string root, CancellationToken cancellationToken)
        {
            ValidateAdaptor(root);
            using var json = await GetJsonAsync($"{BaseUrl}/{root}/rest/services", cancellationToken).ConfigureAwait(false);
            var values = new List<LookupItem>
            {
                new LookupItem(string.Empty, "Root")
            };
            if (json.RootElement.TryGetProperty("folders", out var folders))
            {
                foreach (var folderValue in folders.EnumerateArray())
                {
                    ...
                    {
                        values.Add(new LookupItem(folder, folder));
                    }
                }
            }
        ...
        /// </summary>
        /// <param name="root">The selected ArcGIS Web Adaptor.</param>
        /// <param name="folder">The selected folder path.</param>
        /// <param name="cancellationToken">The cancellation token.</param>
        /// <returns>The available service selections.</returns>
        [HttpGet("services")]
        public async Task<LookupResponse> GetServices([FromQuery] string root, [FromQuery] string folder, CancellationToken cancellationToken)
        {
        ...
        [HttpGet("tables")]
        public async Task<LookupResponse> GetTables([FromQuery] string root, [FromQuery] string folder, [FromQuery] string service, CancellationToken cancellationToken)
        {
