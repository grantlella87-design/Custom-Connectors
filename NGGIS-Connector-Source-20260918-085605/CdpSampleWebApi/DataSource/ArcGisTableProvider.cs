...
namespace CdpSampleWebApi
{
    ...
    public sealed class ArcGisTableProvider : ITableProvider
    {
        ...
        {
            return GetTableValueAsync(dataset, tableName, false, false, "4326", cancellationToken);
        }

        public async Task<TableValue> GetTableValueAsync(string dataset, string tableName, bool decodeDomainsAndSubtypes, bool returnGeometry, string outputSpatialReference, CancellationToken cancellationToken = default)
        {
            ValidateDataset(dataset);
            var layer = await GetLayerAsync(tableName, cancellationToken).ConfigureAwait(false);
            ...
            var form = new Dictionary<string, string>
            {
                ...
                ["outSR"] = string.IsNullOrWhiteSpace(outputSpatialReference) ? "4326" : outputSpatialReference,
            ...
            if (json.RootElement.TryGetProperty("features", out var features))
            {
                foreach (var feature in features.EnumerateArray())
                {
                    ...
                    if (feature.TryGetProperty("attributes", out var attributes))
                    {
                        ...
                        {
                            foreach (var field in layer.Fields.Where(field => !string.Equals(field.Name, layer.SubtypeField, StringComparison.OrdinalIgnoreCase)))
                            {
                                row.TryGetValue(field.Name, out var value);
                                row[field.Name] = DecodeDomainValue(layer, field, row, value);
                            }

                            if (!string.IsNullOrWhiteSpace(layer.SubtypeField))
                            {
                                var subtypeField = layer.Fields.FirstOrDefault(field => string.Equals(field.Name, layer.SubtypeField, StringComparison.OrdinalIgnoreCase));
                                if (subtypeField != null)
                                {
                                    row.TryGetValue(subtypeField.Name, out var subtypeValue);
                                    row[subtypeField.Name] = DecodeDomainValue(layer, subtypeField, row, subtypeValue);
                                }
                            }
                        }
                    }
                    ...
                    if (returnGeometry)
                    {
                        ...
                        if (json.RootElement.TryGetProperty("spatialReference", out var responseSpatialReference) && responseSpatialReference.TryGetProperty("latestWkid", out var latestWkid))
                        {
                            spatialReferenceWkid = latestWkid.GetRawText();
                        }
                        else if (json.RootElement.TryGetProperty("spatialReference", out responseSpatialReference) && responseSpatialReference.TryGetProperty("wkid", out var wkid))
                        {
                            spatialReferenceWkid = wkid.GetRawText();
                        }
                        row["SpatialReferenceWkid"] = spatialReferenceWkid;
                    }
                    rows.Add(row);
                }
            }
            ...
            foreach (var row in rows)
            {
                ...
                if (returnGeometry)
                {
                    row.TryGetValue("GeometryJson", out var geometryJson);
                    row.TryGetValue("SpatialReferenceWkid", out var spatialReferenceWkid);
                    values.Add(new NamedValue("GeometryJson", geometryJson == null ? FormulaValue.NewBlank(FormulaType.String) : FormulaValue.New(geometryJson.ToString() ?? string.Empty)));
                    values.Add(new NamedValue("SpatialReferenceWkid", spatialReferenceWkid == null ? FormulaValue.NewBlank(FormulaType.String) : FormulaValue.New(spatialReferenceWkid.ToString() ?? string.Empty)));
                }
