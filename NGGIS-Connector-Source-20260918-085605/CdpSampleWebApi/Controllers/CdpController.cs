...
namespace CdpSampleWebApi.Controllers
{
    ...
    public class CdpController : ControllerBase
    {
        ...
        /// <summary>
        /// Gets the health status of the enhanced GIS API.
        /// </summary>
        /// <returns>The API health response.</returns>
        [HttpGet]
        [Route("health")]
        public IActionResult Health()
        {
            ...
            {
                status = "Healthy",
                startupTimeUtc = Program.StartupTime,
        ...
        public async Task<ActionResult<GetTableResponse>> GetTableInfo(
            string dataset,
            string tableName,
            [FromQuery(Name = "api-version")] string apiVersion)
        {
            ...
            catch (InvalidOperationException ex) when (ex.Message == "Invalid PROD ArcGIS table identifier.")
            {
        ...
        public async Task<GetItemsResponse> GetItems(
            ...
            [FromQuery] bool decodeDomainsAndSubtypes = true,
            [FromQuery] bool returnGeometry = false,
            [FromQuery] string outputSpatialReference = "4326")
        {            
            var provider = GetProvider();
            TableValue tableValue = provider is ArcGisTableProvider arcGisProvider
                ? await arcGisProvider.GetTableValueAsync(dataset, tableName, decodeDomainsAndSubtypes, returnGeometry, outputSpatialReference, CancellationToken.None).ConfigureAwait(true)
                : await provider.GetTableValueAsync(dataset, tableName, CancellationToken.None).ConfigureAwait(true);
        ...
        private ITableProvider GetProvider()
        {
            ...
            if (_providerFactory is TrivialTableProviderFactory || _providerFactory is ArcGisTableProviderFactory)
            {
