    ...
    public static class OData2XrmConverter
    {
        ...
        public static FxFilterExpression Parse(SingleValueNode node)
        {
            if (node is BinaryOperatorNode bin)
            {
                ...
                // Field op Value
                SingleValueNode leftNode = bin.Left;
                while (leftNode is ConvertNode || leftNode is SingleValueCastNode)
                {
                    if (leftNode is ConvertNode convertedLeftNode)
                    {
                        leftNode = convertedLeftNode.Source;
                    }
                    else if (leftNode is SingleValueCastNode castLeftNode)
                    {
                        leftNode = castLeftNode.Source;
                    }
                }

                string leftName = null;
                if (leftNode is SingleValuePropertyAccessNode propertyNode)
                {
                    leftName = propertyNode.Property.Name;
                }
                else if (leftNode is SingleValueOpenPropertyAccessNode openPropertyNode)
                {
                    leftName = openPropertyNode.Name;
                }
                ...
                if (leftName != null)
                {
                    ...
                    // OData can wrap numeric constants in conversion nodes.
                    SingleValueNode rightNode = bin.Right;
                    while (rightNode is ConvertNode || rightNode is SingleValueCastNode)
                    {
                        if (rightNode is ConvertNode convertedRightNode)
                        {
                            rightNode = convertedRightNode.Source;
                        }
                        else if (rightNode is SingleValueCastNode castRightNode)
                        {
                            rightNode = castRightNode.Source;
                        }
                    }

                    if (rightNode is ConstantNode c)
                    {
                ...
                throw new NotImplementedException($"Unhandled binary node. Operator={bin.OperatorKind}; Left={bin.Left.GetType().FullName}; LeftKind={bin.Left.Kind}; Right={bin.Right.GetType().FullName}; RightKind={bin.Right.Kind}");
            }
